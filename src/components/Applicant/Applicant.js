...
//       // Merge the fetched data with current form data
//       const updatedInfo = { ...formData.applicantInfo, ...data };
//       const updatedContact = { ...formData.applicantContact, ...data };
  
//       // Re-validate both sections with the merged data
//       const infoErrors = validateApplicantInfo(updatedInfo);
//       const contactErrors = validateApplicantContact(updatedContact);
  
//       // Update form data and errors together
//       setFormData({
//         applicantInfo: updatedInfo,
...
//     } catch (error) {
//       alert(error.message);
//       // Optionally set an error on the ID field
//       // setErrors(prev => ({
//       //   ...prev,
//       //   applicantInfo: { ...prev.applicantInfo, idNo: error.message }
//       // }));
//     } finally {
//       setSearchLoading(false);
//     }
...
//       // Update errors immediately (functional update to avoid any closure issues)
        ...
        <div className="relative flex flex-col w-full border rounded-lg bg-gray-50">
          {currentIndex === 0 ? (
            <ApplicantInfo
              ...
              isModify={isModify}
              searchLoading={searchLoading}
              errors={errors.applicantInfo}
            />
          ) : (
            <ApplicantContact
              onInputChange={(data) => handleInputChange("applicantContact", data)}
              data={formData.applicantContact}
              errors={errors.applicantContact}
            />
          )}
