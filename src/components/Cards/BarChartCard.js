...
//new code with dynamic backend data 
...
const BarChartCard = () => {
    ...
    const fetchJobTypeCounts = async () => {
      try {
        const response = await fetch(
          `${baseUrl}/api/order-cards/job-type-counts`
        );

        if (!response.ok) {
          throw new Error("Failed to fetch job type counts");
        }
  ...
  // 🟢 Prepare chart data
  const labels = data.map(item => item.jobType);
  ...
  return (
    ...
    >
      <h2 className="text-sm font-semibold mb-2">
        Application Status by Job Type
      </h2>
