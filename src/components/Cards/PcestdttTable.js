  ...
  return (
    <div className="overflow-x-auto border rounded-lg">
      <table className="min-w-full bg-white">
        <thead className="bg-gray-100">
          <tr>
            {headers.map((header) => (
              <th
                key={header}
                className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase border-b"
              >
                {header}
              </th>
            ))}
        ...
        <tbody>
          {data.map((item, index) => (
            <tr key={item.resCd} className={index % 2 === 0 ? "bg-gray-50" : "bg-white hover:bg-gray-100"}>
              <td className="px-4 py-2 text-sm text-gray-700 border-b">{item.resCd}</td>
              {[
                "resType",
                "uom",
                "unitPrice",
                ...
                <td
                  key={field}
                  className="px-4 py-2 text-sm text-gray-700 border-b cursor-pointer hover:text-[#7c0000]"
                  onClick={() => startEditing(item, field, item[field])}
                >
                  {editingCell?.resCd === item.resCd && editingCell?.field === field ? (
                    <input
              ...
              <td className="px-4 py-2 text-sm border-b">
                <button
                  type="button"
                  onClick={() => handleDeleteClick(item)}
                  className="bg-[#7c0000] text-white text-xs px-2 py-1 rounded hover:bg-[#a00000] focus:outline-none focus:ring-2 focus:ring-[#7c0000] focus:ring-opacity-50"
                >
                  Delete
                </button>
              </td>
