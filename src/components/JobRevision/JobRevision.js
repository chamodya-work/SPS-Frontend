...
const JobRevision = ({
  ...
  const handlePrevious = () => {
    if (currentIndex > 0) {
      // Unmark the previous tab when going back
      setCompletedTabs((prev) => {
        const newTabs = [...prev];
        // Clear completion status for the tab we're leaving
        if (currentIndex - 1 >= 0) {
        ...
        {tabs.map((tab, index) => (
          <div key={index} className="relative flex-1 flex flex-col items-center">
            ...
            <div
              ...
              onClick={() => {
                // Only allow clicking on tabs that are accessible
                if (index <= currentIndex || completedTabs[index - 1] || index === 0) {
                  setCurrentIndex(index);
                  // Optionally, you might want to reset completion status when clicking directly
                  // But for now, we'll keep the existing behavior
                }
              }}
            >
              {index + 1}
