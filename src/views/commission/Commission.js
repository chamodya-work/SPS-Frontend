...
export default function Commission({ color }) {
  ...
  const fetchCommissionDetails = async () => {
    try {
      ...
      // //only get commision detail using status=4
      ...
      //OLD :updated commision fetching details using status=4 and filter using userId 
      //NOW IN HERE status=1 and filter using userId 
      const response = await fetch(`${baseUrl}/api/commission/details/user/create?userId=${encodeURIComponent(userId.trim())}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: "Basic " + btoa("user:admin123"),
  ...
  return (
    ...
    maximumFractionDigits: 2
                        ...
                        {/* this is commented because we dont need status when create order card */}
                        ...
                        <td className="table-cell">
                          <div className="action-buttons">
                            ...
                            <Link
                              to={`/admin/commission/${encodeURIComponent(
                                c.estimateNo
                              )}`}
                            >
                            ...
                            >
                              Create Order Card {expandedRows.has(c.estimateNo) ? '▲' : '▼'}
                            </button>
                          </div>
                        </td>
                      </tr>
                      {/* Expanded row for OrderCardPopupNew */}
