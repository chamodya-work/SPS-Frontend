...
// export default function CommissionValidate({ color }) {
...
//   // Add this function getting related api according to userlevel
//   const getApiEndpointByUserLevel = (userLevel) => {
//     const endpointMap = {
//       'deo': 'edit',
//       'ee': 'validate',
//       'es': 'verify',
...
//       //  console.log("meka deptId :", deptId);

//        // Get the appropriate API endpoint based on user level
//       const apiEndpoint = getApiEndpointByUserLevel(userLevel);

//       const response = await fetch(`${baseUrl}/api/commission/details/user/validate?userId=${encodeURIComponent(userId)}`, {
//         method: "GET",
//         headers: {
...
//   const updateEstimateStatus12 = async (estimateNo, deptId) => {
//     try {
//       const response = await fetch(
//         `${baseUrl}/api/pcesthmt/update-status-to-12?estimateNo=${encodeURIComponent(estimateNo)}&deptId=${encodeURIComponent(deptId)}`,
//         {
//           method: "PUT",
//           headers: {
//             "Content-Type": "application/json",
...
//                             <Link
//                               to={`/admin/commission/validate/${encodeURIComponent(
//                                 c.estimateNo
//                               )}`}
//                             >
...
//                               onClick={() => updateEstimateStatus12(c.estimateNo,c.deptId)}
//                             >
...
//                             <button 
//                               className="view-button"
//                               onClick={() => updateEstimateStatus10(c.estimateNo,c.deptId)}
//                             >
//                               {/* Edit Order Card {expandedRows.has(c.estimateNo) ? '▲' : '▼'} */}
//                             Reject
//                             </button>
...
//this is testing code in i trying to modify getApiEndpointByUserLevel logic 
...
export default function CommissionValidate({ color }) {
  ...
  const [userDeptType, setUserDeptType] = useState(null); // Add this store user deptType
  const [userLevel, setUserLevel] = useState(null); // Add this line to store user level


  console.log("this is new userLevel: ",userLevel);
  console.log("this is new userDeptType: ",userDeptType)
  ...
  const getApiEndpointByUserLevel = async (userLevel, deptId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUserDeptType(deptType); // Store department type
  ...
  // Add this function to handle the forward action dynamically for when click 
  //forward to area button
  const handleForwardAction = async (estimateNo, deptId) => {
    try {
      // Check if user is EE and get department type
      if (userLevel?.toLowerCase() === 'ee') {
        if (userDeptType === 'AREA') {
          ...
          toast.error("Cannot determine forwarding action. Unknown department type.");
        }
      } else {
        // For non-EE users, use the default forward action
        await updateEstimateStatus12(estimateNo, deptId);
      }
    } catch (error) {
      console.error("Error in forward action:", error);
      toast.error("Failed to forward order card!");
    }
  };

  // Add this function to handle the backward action dynamically for when click 
  //reject button
  const handleBackwardAction = async (estimateNo, deptId) => {
    try {
      // Check if user is EE and get department type
      if (userLevel?.toLowerCase() === 'ee') {
        if (userDeptType === 'AREA') {
  ...
  const fetchCommissionDetails = async () => {
    ...
    const userId = sessionStorage.getItem("userId");
    const userLevelFromStorage = sessionStorage.getItem("userLevel");
    const deptId = sessionStorage.getItem("deptId");
    
    // Store user level in state
    setUserLevel(userLevelFromStorage);
    
    
    console.log("User ID:", userId, "User Level:", userLevelFromStorage, "Dept ID:", deptId);

       // Get the appropriate API endpoint based on user level and department type
    const apiEndpoint = await getApiEndpointByUserLevel(userLevelFromStorage, deptId);

      // Use the determined endpoint in the API call
  ...
  const updateEstimateStatus14 = async (estimateNo, deptId) => {
    try {
      const response = await fetch(
        `${baseUrl}/api/pcesthmt/update-status-to-14?estimateNo=${encodeURIComponent(estimateNo)}&deptId=${encodeURIComponent(deptId)}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
                        ...
                        <td className="table-cell">
                          <div className="action-buttons">
                            ...
                            <button 
                              className="view-button accept"
                              // onClick={() => updateEstimateStatus12(c.estimateNo,c.deptId)}
                              onClick={() => handleForwardAction(c.estimateNo,c.deptId)}
                            >
                              {/* Edit Order Card {expandedRows.has(c.estimateNo) ? '▲' : '▼'} */}
                            Forward To Area
                            </button>
                            ...
                            <button 
                              className="view-button"
                              // onClick={() => updateEstimateStatus10(c.estimateNo,c.deptId)}
                              onClick={()=>handleBackwardAction(c.estimateNo,c.deptId)}
                            >
                              {/* Edit Order Card {expandedRows.has(c.estimateNo) ? '▲' : '▼'} */}
                            Reject
                            </button>
