...
export default function StandardRates() {
  const [rates, setRates] = useState([]);
  const [editingId, setEditingId] = useState(null);
  const [updatedCost, setUpdatedCost] = useState({});
  const [newRate, setNewRate] = useState({
  ...
  const totalPages = Math.ceil(rates.length / rowsPerPage) || 1;
  const currentData = rates.slice(
    (currentPage - 1) * rowsPerPage,
    currentPage * rowsPerPage
  );
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded bg-white">
        <div className="rounded-t mb-0 px-6 py-4 border-0 flex justify-between items-center">
          <h3 className="font-semibold text-lg text-gray-800">Standard Rates</h3>
          <button
            onClick={handleAddRow}
        ...
        <div className="block w-full overflow-x-auto">
          <table className="items-center w-full bg-transparent border-collapse">
            <thead>
              <tr>
                <th className="px-6 align-middle border border-solid py-3 text-xs uppercase font-semibold text-left bg-gray-50 text-gray-600 border-gray-200">
                  ID
                </th>
                <th className="px-6 align-middle border border-solid py-3 text-xs uppercase font-semibold text-left bg-gray-50 text-gray-600 border-gray-200">
                  UOM
                  ...
                  <tr key={rate.lineSectionTypeId}>
                    ...
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-gray-700">
                      {editingId === rate.lineSectionTypeId ? (
                        <input
                          ...
                          className="px-2 py-1 w-full border border-gray-300 rounded text-sm bg-white focus:outline-none focus:border-[#7c0000] focus:shadow-[0_0_0_2px_rgba(124,0,0,0.1)] transition-all duration-150"
                        />
                      ) : (
                        rate.standardCost
                      )}
                    ...
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4">
                      {editingId === rate.lineSectionTypeId ? (
                        <button
                          onClick={() => handleUpdate(rate.lineSectionTypeId)}
                          className="bg-green-500 text-black text-sm px-3 py-1 rounded shadow hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-opacity-50"
                        >
                          Save
                        </button>
                      ) : (
                        <button
                          onClick={() =>
                            handleEdit(rate.lineSectionTypeId, rate.standardCost)
                          }
                          className="bg-[#7c0000] text-white text-sm px-3 py-1 rounded shadow hover:bg-[#a00000] focus:outline-none focus:ring-2 focus:ring-[#7c0000] focus:ring-opacity-50"
                        >
                          Edit
                        </button>
                      )}
                ...
                <tr>
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm p-4">
                    <input
                      type="text"
                      onChange={(e) =>
                        handleNewRateChange("lineSectionTypeId", e.target.value)
                      }
                      className="px-2 py-1 w-full border border-gray-300 rounded text-sm bg-white focus:outline-none focus:border-[#7c0000] focus:shadow-[0_0_0_2px_rgba(124,0,0,0.1)] transition-all duration-150"
                    />
                  </td>
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm p-4">
                    <input
                      type="text"
                      onChange={(e) => handleNewRateChange("uom", e.target.value)}
                      className="px-2 py-1 w-full border border-gray-300 rounded text-sm bg-white focus:outline-none focus:border-[#7c0000] focus:shadow-[0_0_0_2px_rgba(124,0,0,0.1)] transition-all duration-150"
                    />
                  </td>
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm p-4">
                  ...
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm p-4">
                    <button
                      onClick={handleSaveNewRow}
                      className="bg-green-500 text-white text-sm px-3 py-1 rounded shadow hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-opacity-50"
                    >
                      Save
                    </button>
                  </td>
        ...
        <div className="flex justify-end mt-4 space-x-2 pb-2 px-6">
          <button
            onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))}
            disabled={currentPage === 1}
            className="bg-[#7c0000] text-white text-sm px-4 py-2 rounded shadow hover:bg-[#a00000] focus:outline-none focus:ring-2 focus:ring-[#7c0000] focus:ring-opacity-50 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Previous
          </button>
